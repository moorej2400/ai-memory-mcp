...
def test_change_journal_uses_the_committed_batch_revision(
    artifact_store: ArtifactStore,
) -> None:
    first = artifact_store.apply_batch(message_batch(source_sequence=1))
    replay = artifact_store.apply_batch(
        message_batch(batch_id="batch-replay", source_sequence=1)
    )
    ...
    assert first.artifacts_changed == 2
    assert replay.artifacts_changed == 0
    assert second.artifacts_changed == 1

    with connect_artifact_db(
        artifact_store.settings.artifact_db, read_only=True
    ) as connection:
        counter = int(
            connection.execute(
        ...
        rows = connection.execute(
            "SELECT revision, artifact_id, latest_event_id, operation, "
            "affected_parent_id FROM artifact_change_journal "
    ...
    assert len({(row["revision"], row["artifact_id"]) for row in rows}) == len(rows)
